...
class IFVPs():
    def __init__(self, grads, block_size, block_batch, damping, check=False):
        self.ifvps = []
        self.n = grads.shape[1]
        self.block_size = block_size
        self.block_batch = block_batch
        self.m = self.block_size * self.block_batch
        for i in range(0, self.n, self.m):
            self.ifvps.append(
                IFVP(grads[:, i:i + self.m], self.block_size, damping))

    def diag(self):
        return torch.hstack([x.diag() for x in self.ifvps])

    def __call__(self, x):
        d = []
        for i in range(0, self.n, self.m):
            d.append(self.ifvps[i // self.m](x[i:i + self.m]))
        return torch.hstack(d)
